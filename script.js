    ...
    if (checkSpecialButton && dataTableDiv) {
        checkSpecialButton.addEventListener('click', () => {
            ...
            if (currentClassNumber === 1 || currentClassNumber === 2) {
                ...
                // Cấu hình kiểm tra cho Lớp 3 (dựa trên cấu trúc bảng)
                specialChecks = [
                    { subject: "Toán", rateIndex: 6, abilityIndex: 26, color: "yellow" }, // Toán: Mức đạt được (6) vs Tính toán (26)
                    { subject: "Tiếng Việt", rateIndex: 8, abilityIndex: 25, color: "orange" }, // Tiếng Việt: Mức đạt được (8) vs Ngôn ngữ (25)
                    { subject: "Tự nhiên và xã hội", rateIndex: 10, abilityIndex: 27, color: "brown" }, // TNXH: Mức đạt được (10) vs Khoa học (27)
                    { subject: "TH-CN (Tin học)", rateIndex: 14, abilityIndex: 29, color: "lightblue" }, // TH-CN (Tin học): Mức đạt được (14) vs Tin học (29)
                    { subject: "TH-CN (Công nghệ)", rateIndex: 17, abilityIndex: 28, color: "lightgreen" }, // TH-CN (Công nghệ): Mức đạt được (17) vs Công nghệ (28)
                    { subject: "Giáo dục thể chất", rateIndex: 21, abilityIndex: 31, color: "violet" } // GDTC: Mức đạt được (22) vs Thể chất (31)
                ];
            ...
            rows.forEach(row => {
                ...
                // Kiểm tra năng lực Thẩm mĩ cho Lớp 3 (ô index 30) dựa trên Âm nhạc (index 19) và Mĩ thuật (index 20)
                if (checkThamMyClass3 && cells.length > 30 && cells.length > 19 && cells.length > 20) {
                    const amNhac = cells[19].textContent.trim();
                    const miMy = cells[20].textContent.trim();
                    const thamMyCell = cells[30];
                    const amNhacCell = cells[19]; // Lấy ô Âm nhạc
                    const miMyCell = cells[20]; // Lấy ô Mĩ thuật
                ...
                if (checkThamMy && cells.length > 33 && cells.length > 21 && cells.length > 22) {
                    const amNhac = cells[21].textContent.trim();
                    const miMy = cells[22].textContent.trim();
        ...
        editButton.addEventListener('click', () => {
            ...
            if (currentClassNumber === 1 || currentClassNumber === 2) {
                ...
                subjectPairs = [
                 ...
                 specialChecks = [
                    { subject: "Toán", rateIndex: 6, abilityIndex: 26 }, // Toán: Mức đạt được (6) vs Tính toán (26)
                    { subject: "Tiếng Việt", rateIndex: 8, abilityIndex: 25 }, // Tiếng Việt: Mức đạt được (8) vs Ngôn ngữ (25)
                    { subject: "Tự nhiên và xã hội", rateIndex: 10, abilityIndex: 27 }, // TNXH: Mức đạt được (10) vs Khoa học (27)
                    { subject: "TH-CN (Tin học)", rateIndex: 14, abilityIndex: 29 }, // TH-CN (Tin học): Mức đạt được (14) vs Tin học (29)
                    { subject: "TH-CN (Công nghệ)", rateIndex: 17, abilityIndex: 28 }, // TH-CN (Công nghệ): Mức đạt được (17) vs Công nghệ (28)
                    { subject: "Giáo dục thể chất", rateIndex: 21, abilityIndex: 31 } // GDTC: Mức đạt được (22) vs Thể chất (31)
                ];
                checkThamMy = false;
            ...
            rows.forEach(row => {
                ...
                if ((currentClassNumber === 1 || currentClassNumber === 2) && cells.length > 24 && cells.length > 14 && cells.length > 15) {
                 ...
                 // Sửa năng lực Thẩm mĩ cho Lớp 3
                if (checkThamMyClass3 && cells.length > 30 && cells.length > 19 && cells.length > 20) {
                    const amNhac = cells[19].textContent.trim(); // Lấy mức đạt được Âm nhạc (có thể đã sửa)
                    const miMy = cells[20].textContent.trim(); // Lấy mức đạt được Mĩ thuật (có thể đã sửa)
                    const thamMyCell = cells[30];
                    const amNhacCell = cells[19];
                    const miMyCell = cells[20];
